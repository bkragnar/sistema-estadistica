        ...
        <div class="row">
            <div>
                ...
                <select class="row ml-3" id="anio_eg_cne" name="anio_eg_cne">
                    ...
                    <?php
                    $sql_anio_cne = $connection->query("SELECT distinct(anio_eg) FROM egreso_le WHERE tipo_le_eg=1");
                    while ($res_provincia = mysqli_fetch_array($sql_provincia)) {
                        echo '<option value="' . $res_provincia[0] . '">' . $res_provincia[0] . '</option>';
                    }
                    ?>
                </select>
            </div>
            <div class="ml-5">
                ...
                <select class="row ml-2" id="tipo_estable_cne" name="tipo_estable_cne">
                    <option value="0" disabled>Seleccione:</option>
                    <?php
